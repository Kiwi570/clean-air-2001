...
import { MessageCircle, Send, ChevronLeft, Search, MoreVertical, Phone, Video, Check, CheckCheck, Smile, Image, Paperclip } from 'lucide-react'
...
function Messages() {
  ...
  const conversations = getConversationsWithPreview('cleaner')
  ...
  useEffect(() => {
    if (selectedConversation) {
      markConversationAsRead(selectedConversation.id, 'cleaner')
    }
  ...
  const handleSend = () => {
    ...
    sendMessage(selectedConversation.id, newMessage.trim(), 'cleaner')
    setNewMessage('')
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-surface-900 font-display">
              {selectedConversation ? selectedConversation.hostName : 'Messages'}
            </h1>
            {selectedConversation && (
              <p className="text-sm text-surface-500">{selectedConversation.propertyName}</p>
            )}
            {!selectedConversation && (
        ...
        <div className={cn(
          ...
          <div className="relative mb-4">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-surface-50 border-2 border-surface-200 rounded-xl text-surface-900 placeholder-surface-400 focus:outline-none focus:border-brand-500 focus:bg-white transition-all"
            />
          </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className={cn(
                          ...
                          conv.unreadCount > 0 ? 'text-brand-600 font-semibold' : 'text-surface-400'
                        )}>
                          {conv.lastMessage && formatTime(conv.lastMessage.timestamp)}
                        </span>
                      </div>
                      <p className="text-xs text-brand-600 mb-1">{conv.propertyName}</p>
                      {conv.lastMessage && (
                        <p className={cn(
                          'text-sm truncate',
                          conv.unreadCount > 0 ? 'text-surface-700 font-medium' : 'text-surface-500'
                        )}>
                          {conv.lastMessage.from === 'cleaner' && (
          ...
          {selectedConversation ? (
              ...
              <Card className="flex-1 overflow-hidden p-0 flex flex-col">
                <div className="flex-1 overflow-y-auto p-4 lg:p-6 space-y-4">
                  {currentMessages.map((message, index) => {
                    const isMe = message.from === 'cleaner'
                  ...
                  <div className="flex items-end gap-3">
                    ...
                    <div className="flex-1 relative">
                      <textarea
                        ...
                        rows={1}
                        className="w-full px-4 py-3 pr-12 bg-white border-2 border-surface-200 rounded-2xl text-surface-900 placeholder-surface-400 focus:outline-none focus:border-brand-500 resize-none transition-all"
            ...
            <Card className="flex-1 flex items-center justify-center">
              <div className="text-center">
                <div className="w-24 h-24 bg-gradient-to-br from-brand-100 to-accent-100 rounded-3xl flex items-center justify-center mx-auto mb-6 animate-float">
                ...
                <p className="text-surface-500 max-w-sm">
                  Sélectionnez une conversation pour voir vos échanges avec les hôtes.
                </p>
              </div>
            </Card>
          )}
